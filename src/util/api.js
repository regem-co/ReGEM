...
export const initiatePrivileged = body => {
  ...
  if (body.orderData && body.orderData.restOfShoppingCartItems) {
    // Destructure 
    const { restOfShoppingCartItems, ...orderDataWithoutCartItem } = body.orderData;

    // Create a new object 
    const filteredBody = {
      ...body,
      orderData: orderDataWithoutCartItem,  // Use the updated orderData
    };


    return post('/api/initiate-privileged', filteredBody);
  }

  return post('/api/initiate-privileged', body);
};

export const initiatePrivilegedAffirm = body => {
  return post('/api/initiate-privileged-affirm', body);
};

export const transitionPrivilegedAffirm = body => {
  return post('/api/transition-privileged-affirm', body);
};
...
export const addToMailchimp = body => {
  return post('/api/mailchimp', body);
};

export const createListing = ({ listingId }) => {
  const options = {
    method: methods.GET,
  };
  const queryParams = {
  ...
  const uri = `/api/listings/create${queryString}`;
  return request(uri, options);
};

export const requestShippingLabel = body => {
  return post('/api/ups/request-shipping-label1', body);
};