...
import { addToMailchimp, createUserWithIdp } from '../util/api';
...
export const login = (username, password, type) => (dispatch, getState, sdk) => {
  type = (type !== 'signup') ? 'signin' : type;
  ...
  return sdk
    ...
    .then(() => dispatch(fetchCurrentUser(type)))
...
export const signup = params => (dispatch, getState, sdk) => {
  ...
  return sdk.currentUser
    ...
    .then(() => dispatch(login(email, password, 'signup')))
    .then(() => {
      if (!params.hasAcceptedNewsletter) return;

      addToMailchimp({ email }).then().catch();
    })
    .catch(e => {
