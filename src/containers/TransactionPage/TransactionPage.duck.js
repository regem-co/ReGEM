...
export const OFFER_ACTION_REQUEST = 'app/TransactionPage/OFFER_ACTION_REQUEST';
export const OFFER_ACTION_SUCCESS = 'app/TransactionPage/OFFER_ACTION_SUCCESS';
export const OFFER_ACTION_ERROR = 'app/TransactionPage/OFFER_ACTION_ERROR';
...
export default function checkoutPageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case OFFER_ACTION_REQUEST:
      return { ...state, offerActionInProgress: true, offerActionError: null };
    case OFFER_ACTION_SUCCESS:
      return { ...state, offerActionInProgress: false };
    case OFFER_ACTION_ERROR:
      return { ...state, offerActionInProgress: false, offerActionError: payload };
...
const offerActionRequest = () => ({ type: OFFER_ACTION_REQUEST });
const offerActionSuccess = () => ({ type: OFFER_ACTION_SUCCESS });
const offerActionError = e => ({ type: OFFER_ACTION_ERROR, error: true, payload: e });
...
export const acceptOfferByProvider = tx => (dispatch, getState, sdk) => {
  const id = tx.id;
  ...
  return sdk.transactions
    .transition(
      { id, transition: TRANSITION_OFFER_ACCEPTED_BY_PROVIDER, params: {} },
      { expand: true }
    )
    .then(response => {
      dispatch(addMarketplaceEntities(response));
    ...
    .catch(e => {
      dispatch(offerActionError(storableError(e)));
      log.error(e, 'accept-offer-by-provider-failed', {
        txId: id,
        transition: TRANSITION_OFFER_ACCEPTED_BY_PROVIDER,
      });
      throw e;
    });
};

// this should be Accept & Pay (or Accept & Pay with Affirm)
export const acceptOfferByCustomer = tx => (dispatch, getState, sdk) => {
  const id = tx.id;
  ...
  return sdk.transactions
    .transition(
      { id, transition: TRANSITION_OFFER_ACCEPTED_BY_CUSTOMER, params: {} },
      { expand: true }
    )
    .then(response => {
      dispatch(addMarketplaceEntities(response));
    ...
    .catch(e => {
      ...
      log.error(e, 'accept-offer-by-provider-failed', {
        txId: id,
        transition: TRANSITION_OFFER_ACCEPTED_BY_CUSTOMER,
      });
      throw e;
    });
};

export const declineOfferByProvider = tx => (dispatch, getState, sdk) => {
  const id = tx.id;
  ...
  return sdk.transactions
    .transition(
      { id, transition: TRANSITION_OFFER_DECLINED_BY_PROVIDER, params: {} },
      { expand: true }
    )
    .then(response => {
      dispatch(addMarketplaceEntities(response));
    ...
    .catch(e => {
      dispatch(offerActionError(storableError(e)));
      log.error(e, 'decline-offer-by-provider-failed', {
        txId: id,
        transition: TRANSITION_OFFER_DECLINED_BY_PROVIDER,
      });
      throw e;
    });
};

export const makeOfferByProvider = (role, tx, newOfferValue) => (dispatch, getState, sdk) => {

  const existingOffer = tx.attributes.protectedData.proposedPriceAmount;
  const offersHistory = tx.attributes.protectedData.offersHistory || { offersHistory: [] };
  offersHistory.offersHistory.push({
    offer: existingOffer,
    date: new Date().toISOString(),
    role: role
  });

  const providerSpecificTransition =
    //tx.attributes.lastTransition === TRANSITION_INITIAL_OFFER
    //? 
    TRANSITION_OFFER_MADE_BY_PROVIDER;
  //: TRANSITION_OFFER_PROVIDER_PENDING;

  const id = tx.id;
  //const listingId = tx.listing.id;
  const transition =
    role === 'customer' ? TRANSITION_OFFER_MADE_BY_CUSTOMER : providerSpecificTransition;
  ...
  return sdk.transactions
    .transition(
      {
        ...
        params: {
          protectedData: {
            proposedPriceAmount: newOfferValue + '00',
    ...
    .catch(e => {
      dispatch(offerActionError(storableError(e)));
      log.error(e, 'accept-offer-failed', {
        txId: id,
        transition: transition,
      });
      throw e;
...
export const makeOfferByCustomer = (role, tx, newOfferValue) => (dispatch, getState, sdk) => {

  const id = tx.id;
  //const listingId = tx.listing.id;
  const transition = TRANSITION_COUNTER_OFFER_MADE_BY_CUSTOMER;
