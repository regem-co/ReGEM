...
import css from './ListingsUnbrandedSection.module.css';
...
function ListingsSection(props) {
  const { intl, title, handleDirectSearch } = props;
  ...
  useEffect(() => {
    ...
    sdk.listings
      .query({
        ...
        'fields.listing': ['title', 'geolocation', 'price', 'publicData', 'description', 'createdAt'],
        'fields.user': [
          'profile.displayName',
      ...
      .then(res => {
        ...
        const finalListingsFiltered = finalListings.filter(l => l && l.attributes && l.attributes.createdAt && !l.attributes.publicData.sold);
  ...
  return spotLightListings && spotLightListings.length > 0 ? (
    <div className={css.wrapper}>
      <div className={css.titleWrapper}>
        <h2 className={css.mainTitle}>What's Hot: Designer Gems</h2>
      </div>
      ...
      <a
        ...
        onClick={() => handleDirectSearch({ pub_branded: 'branded' })}
      >
        See all
      </a>
    </div>
