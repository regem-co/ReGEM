...
import { handleCopyListing } from '../utils';
...
export const ManageListingCardComponent = props => {
  ...
  const isOutOfStock = !relisted && currentStock === 0;
  ...
  const handleDeleteListing = () => {
    ...
    onCloseListing(currentListing.id).then(resp => {
                  ...
                  <InlineTextButton
                    ...
                    onClick={event => {
                      ...
                      if (!actionsInProgressListingId) {
                        ...
                        const editListingPath = pathByRouteName('EditListingPage', routes, {
                          id,
                          slug,
  ...
  <InlineTextButton
    ...
    onClick={event => {
      ...
      if (!actionsInProgressListingId) {
        ...
        handleCopyListing(currentListing)
          .finally(() => setIsLoading(false));  // Turn off loader after process is done
      }
    }}
  >
    <FormattedMessage id="ManageListingCard.copyListing" />
  </InlineTextButton>
</MenuItem>
            ...
            {isDraft || isPendingApproval
              ...
              : intl.formatMessage({ id: 'ManageListingCard.manageStock' }, { currentStock })}
                ...
                {/* <MenuItem key="manage-quantity">
                  <InlineTextButton
                    ...
                    onClick={event => {
                      ...
                      if (!actionsInProgressListingId) {
                        onToggleMenu(null);
                        history.push(
                          `/l/${currentListing?.attributes?.title?.replaceAll(' ', '-')}/${
                  ...
                  >
                    <FormattedMessage id="ManageListingCard.manageQuantity" />
                  </InlineTextButton>
                </MenuItem> */}
              </MenuContent>
            </Menu>
        ...
        {isDraft ? (
          <React.Fragment>
            ...
            >
              ...
              <div
                className={css.deleteListingDraftLink}
                name="EditListingPage"
                onClick={() => {
                  onCloseListing(currentListing.id).then(resp => {
                    return sdk.ownListings
              ...
              >
                <FormattedMessage id="ManageListingCard.deleteListingDraft" />
              </div>
            </Overlay>
          </React.Fragment>
        ) : null}
        ...
        {showOutOfStockOverlay ? (
          <Overlay
            message={intl.formatMessage(
              { id: 'ManageListingCard.outOfStockOverlayText2' },
              { listingTitle: title }
            )}
          >
            <span
              className={css.finishListingDraftLink}
              onClick={() => {
                onRelistListing(currentListing.id);
                setRelisted(true);
              }}
            >
              <FormattedMessage id="ManageListingCard.relist" />
            </span>

            <div className={css.closeListingTextLink}>
              {intl.formatMessage(
                { id: 'ManageListingCard.editListingTextOr' },
                {
                  editListingLink: (
                    <NamedLink
                      key="editListingLink"
                    ...
                    >
                      <FormattedMessage id="ManageListingCard.editListingText" />
                    </NamedLink>
                  ),
                }
              )}
