...
export const searchListings = searchParams => (dispatch, getState, sdk) => {
  ...
  return sdk.listings
    ...
    .then(response => {
      ...
      //Filtering out those 
      const filtered = response.data.data.filter(l => {
        const isDeleted = l.attributes.deleted;
        const isClosed = l.attributes.state === "closed";

        const isPocket =
          l.attributes.metadata?.pocketListing === true ||
          l.attributes.metadata?.pocketListing === "true"
          

        return !isDeleted && !isPocket && !isClosed;
      });
      console.log("Filtered array:", filtered.map(l => ({
        id: l.id,
        pocket: l.attributes.metadata?.pocketListing,
      ...
      const newResponse = {
        ...response,
        data: { ...response.data, data: filtered },
      };
...
export const loadData = (params, search) => {
  ...
  return searchListings({
    ...
    'fields.listing': ['title', 'geolocation', 'price', 'publicData', 'currentStock', 'metadata'],
    'fields.user': [
      'profile.displayName',
