...
import KeywordsSearchSection from './KeywordsSearchSection/KeywordsSearchSection';
...
export class SearchPageComponent extends Component {
  ...
  render() {
    ...
    const availableFiltersOptions = isKeywordSearch
    ...
    const availableFilters = filterFiltersOptions(urlQueryParams, availableFiltersOptions);
    const selectedCategory = urlQueryParams.pub_category;

    // N.B. openMobileMap button is sticky.
    // For some reason, stickyness doesn't work on Safari, if the element is <button>

    const originalOrderListings = [...listings];
    const reorderedListings = originalOrderListings.sort(
      (a, b) => b?.currentStock?.attributes?.quantity - a?.currentStock?.attributes?.quantity
    );
