...
const EditListingWizardTab = props => {
  ...
  const automaticRedirectsForNewListingFlow = (tab, listingId) => {
    if (tab !== marketplaceTabs[marketplaceTabs.length - 1]) {
      ...
      if (tab === 'details') {
        redirectAfterDraftUpdate(listingId, params, 'photos', marketplaceTabs, history);
      }
    } else {
      if (typeof window !== 'undefined') {
        window.sessionStorage.setItem('listingUUIDPendingMessage', listingId?.uuid);
      }
  ...
  const onCompleteEditListingWizardTab = (tab, updateValues) => {
    ...
    const skipPublish = updateValues.keepAsDraft;
  ...
  const onSaveDraft = (tab, updateValues, redirectToModifiedUrl) => {
    const onUpdateListingOrCreateListingDraft = isNewURI
      ? (tab, values) => onCreateListingDraft(values)
      : onUpdateListing;
    ...
    onUpdateListingOrCreateListingDraft(tab, updateListingValues)
      .then(r => {
        if (isNewListingFlow) {
          const listingId = r.data.data.id;
          if (redirectToModifiedUrl) {
            redirectToModifiedUrl(updateValues?.title?.replaceAll(' ', '-'), listingId?.uuid);
          }
          return listingId;
          // automaticRedirectsForNewListingFlow(tab, listingId);
        }
  ...
  // const onSaveDraft = (updateValues, redirectToModifiedUrl) => {
    ...
    case DETAILS: {
      ...
      return (
        <EditListingDetailsPanel
          ...
          onSaveDraft={(values, redirectToModifiedUrl) => {
            onSaveDraft(tab, values, redirectToModifiedUrl);
          }}
        />
      );
    }
