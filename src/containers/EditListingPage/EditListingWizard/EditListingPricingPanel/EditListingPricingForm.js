...
export const EditListingPricingFormComponent = props => (
  <FinalForm
    ...
    render={formRenderProps => {
      ...
      const selectedStock = values?.stock;
      const selectedPrice = values?.price?.amount;
      const estimatedEarnings =
        selectedStock &&
        selectedPrice &&
        '$' + ((selectedPrice * 0.8 * selectedStock) / 100)?.toFixed(0) + '.00';
      ...
      return (
        <Form onSubmit={handleSubmit} className={classes}>
          ...
          <div className={css.fieldsWrapper}>
            ...
            <div className={css.field}>
              <p className={css.label}>
                {intl.formatMessage({ id: 'EditListingPricingForm.stockLabel' })}*
              </p>
              <FieldTextInput
                className={css.input}
                id="stock"
