...
import StripePaymentFormAffirm from '../../StripePaymentFormAffirm/StripePaymentFormAffirm';
import { storeData, storedData, clearData } from '../../CheckoutPageSessionHelpers';
import css from './AffirmCheckoutPage.module.css';
...
export default class AffirmCheckoutPage extends Component {
  constructor(props) {
    super(props);
  ...
  loadInitialData() {
    ...
    if (shouldFetchSpeculatedTransaction) {
      ...
      const clientId = url.searchParams.get('payment_intent');
      const redirectStatus = url.searchParams.get('redirect_status');

      const skipStripePaymentIntent = clientSecret || clientId || redirectStatus;
      ...
      fetchSpeculatedTransaction(
        {
          ...
          deliveryMethod,
          ...quantityMaybe,
          orderData: newOrderData,//pageData.orderData,
          ...bookingDatesMaybe(pageData.orderData.bookingDates),
          shippingState: savedState || '',
          paymentMethodTypes: ['affirm'],
          providerId: pageData.listing.author.id.uuid,
          skipStripePaymentIntent: !!skipStripePaymentIntent,
        },
        transactionId
      );
    }
  ...
  handlePaymentIntent(handlePaymentParams) {
    ...
    const fnRequestPayment = fnParams => {
      ...
      const hasPaymentIntents = storedTx.attributes.protectedData && storedTx.attributes.protectedData.stripePaymentIntents;
      ...
      fnParams.paymentMethodTypes = ['affirm'];
      const url = new URL(window.location);
      const clientSecret = url.searchParams.get('payment_intent_client_secret');
      const clientId = url.searchParams.get('payment_intent');
      const redirectStatus = url.searchParams.get('redirect_status');

      fnParams.protectedData = {
        stripePaymentIntents: {
          default: {
            stripePaymentIntentId: clientId,
    ...
    if (redirectStatus && redirectStatus) {
      return handlePaymentIntentCreation(orderParams);
    }

    const routes = routeConfiguration();
    const orderDetailsPath = pathByRouteName('OrderDetailsPage', routes, { id: pageData.transaction.id.uuid });
    const domain = window.location.origin;
    // const fullUrl = `http://localhost:3500/api/order-confirmation`;
    const fullUrl = `${domain}${orderDetailsPath}`;
    // const fullUrl = `${domain}/api/order-confirmation`;

    return stripe.confirmPayment({
      elements: this.state.elementsReference,
      confirmParams: {
  ...
  render() {
    ...
    // Get first and last name of the current user and use it in the StripePaymentFormAffirm to autofill the name field
    const userName =
        ...
        <div className={css.paymentContainer}>
          ...
          {showPaymentForm ? (
            <StripePaymentFormAffirm
              ...
              speculatedTransactionMaybe={speculatedTransactionMaybe}
              setElementsReference={value => this.setState({ elementsReference: value })}
            />
          ) : null}
          {isPaymentExpired ? (
            <p className={css.orderError}>
...
AffirmCheckoutPage.defaultProps = {
