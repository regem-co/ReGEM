...
import css from './StripePaymentFormAffirm.module.css';
  ...
  fonts: [
    {
      ...
      src:
        'local("cardo"), local("cardo"), url("https://assets-sharetribecom.sharetribe.com/webfonts/poppins/Poppins-Medium.ttf") format("truetype")',
    },
  ],
};
 ...
 */
class StripePaymentFormAffirm extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    if (typeof window !== 'undefined') {
      ...
      const shouldRedirect = clientId && clientSecret && redirectStatus;
  ...
  initializeStripeElement(element) {
    const clientSecretFromSpeculatedTx = this.props?.speculatedTransactionMaybe?.attributes
      ?.protectedData?.stripePaymentIntents?.default?.stripePaymentIntentClientSecret;
    const elements =
      clientSecretFromSpeculatedTx &&
      this.stripe.elements({
        clientSecret: clientSecretFromSpeculatedTx,
        ...stripeElementsOptions,
      });

    this.props.setElementsReference(elements);

    if (!this.card) {
      this.card = elements.create('payment', { style: cardStyles });
  ...
  paymentForm(formRenderProps) {
    ...
    const { onShippingStateChange } = this.props;
      ...
      <Form className={classes} onSubmit={handleSubmit} enforcePagePreloadFor="OrderDetailsPage">
        {onShippingStateChange ? (
          <FormSpy
            ...
            onChange={state => onShippingStateChange(state.values?.recipientState)}
          />
        ) : null}
...
StripePaymentFormAffirm.defaultProps = {
  className: null,
  rootClassName: null,
  inProgress: false,
  loadingData: false,
  ...
  pickupLocation: null,
  totalPrice: null,
  onShippingStateChange: null,
};

StripePaymentFormAffirm.propTypes = {
  className: string,
  ...
  totalPrice: string,
  onShippingStateChange: func,
};

export default injectIntl(StripePaymentFormAffirm);
