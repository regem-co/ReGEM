...
import { initiatePrivilegedAffirm, transitionPrivilegedAffirm } from '../../util/api';
...
export const initiateOrderCard = (orderParams, transactionId) => (dispatch, getState, sdk) => {
  ...
  const proposedPriceAmount = orderParams?.proposedPriceAmount;

  if (proposedPriceAmount) {
    transition = TRANSITION_REQUEST_PAYMENT_AFTER_OFFER_ACCEPTED;
  }
  ...
  const orderData = {
    ...
    shippingState: orderParams.shippingState,
  };
...
export const initiateOrderAffirm = (orderParams, transactionId) => (dispatch, getState, sdk) => {
  dispatch(initiateOrderRequest());
  ...
  let transition = TRANSITION_REQUEST_PAYMENT_AFFIRM;
  ...
  if (isTransition && isPrivilegedTransition) {
    // transition privileged
    return transitionPrivilegedAffirm({ isSpeculative: false, orderData, bodyParams, queryParams })
      .then(handleSucces)
      .catch(handleError);
  } else if (isTransition) {
    // transition non-privileged
    ...
    // initiate privileged
    return initiatePrivilegedAffirm({ isSpeculative: false, orderData, bodyParams, queryParams })
      .then(handleSucces)
      .catch(handleError);
  } else {
    // initiate non-privileged
...
export const confirmPaymentCard = orderParams => (dispatch, getState, sdk) => {
...
export const confirmPaymentAffirm = orderParams => (dispatch, getState, sdk) => {
  dispatch(confirmPaymentRequest());
 ...
 */
export const speculateTransactionCard = (orderParams, transactionId) => (
  dispatch,
  getState,
  sdk
) => {
  ...
  // If we already have a transaction ID, we should transition, not
  // initiate.
  const proposedPriceAmount = orderParams.orderData.proposedPriceAmount;
  const isTransition = !!transactionId;
  let transition = isTransition
    ? TRANSITION_REQUEST_PAYMENT_AFTER_ENQUIRY
    : TRANSITION_REQUEST_PAYMENT;
...
export const speculateTransactionAffirm = (orderParams, transactionId) => (
  dispatch,
  getState,
  sdk
) => {
  ...
  if (isTransition && isPrivilegedTransition) {
    // transition privileged
    return transitionPrivilegedAffirm({ isSpeculative: true, orderData, bodyParams, queryParams })
      .then(handleSuccess)
      .catch(handleError);
  } else if (isTransition) {
    // transition non-privileged
    ...
    // initiate privileged
    return initiatePrivilegedAffirm({ isSpeculative: true, orderData, bodyParams, queryParams })
      .then(handleSuccess)
      .catch(handleError);
  } else {
    // initiate non-privileged
